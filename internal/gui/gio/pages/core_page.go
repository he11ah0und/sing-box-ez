...
func (p *CorePage) refreshVersions() {
	...
	if err != nil || ver == "" {
		p.versionText = i18n.T("core.version.not_installed")
	} else {
		p.versionText = i18n.T("core.version.installed") + ver
	}
	p.latestText = i18n.T("core.latest.checking")
}
...
func (p *CorePage) Name() string { return i18n.T("tab.core") }
	...
	return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return layout.Inset{Top: unit.Dp(8)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				return material.H6(p.th, i18n.T("tab.core")).Layout(gtx)
			})
		}),
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
		...
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return layout.Inset{Top: unit.Dp(8)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				return material.Button(p.th, &p.downloadBtn, i18n.T("core.btn.download")).Layout(gtx)
			})
		}),
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return layout.Inset{Top: unit.Dp(4)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				return material.Button(p.th, &p.checkBtn, i18n.T("core.btn.check")).Layout(gtx)
			})
		}),
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return material.CheckBox(p.th, &p.autoRestart, i18n.T("core.auto_restart")).Layout(gtx)
		}),
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return material.CheckBox(p.th, &p.watchLogs, i18n.T("core.watch_core_logs")).Layout(gtx)
		}),
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
		...
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return layout.Inset{Top: unit.Dp(16)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				return material.H6(p.th, i18n.T("core.privileges.title")).Layout(gtx)
			})
		}),
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
...
func (p *CorePage) layoutWindowsPrivileges(gtx layout.Context) layout.Dimensions {
	...
	if p.privilegeState.ShowRestartAdminBtn {
		children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return layout.Inset{Top: unit.Dp(8)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				return material.Button(p.th, &p.restartAdminBtn, i18n.T("core.btn.restart_admin")).Layout(gtx)
			})
		}))
	}
...
func (p *CorePage) layoutLinuxPrivileges(gtx layout.Context) layout.Dimensions {
	label := i18n.T("core.mode.admin")
	if p.privilegeMode == "setcap" {
		label = i18n.T("core.mode.setcap")
	}
...
func (p *CorePage) openPrivilegePicker() {
	...
	p.dialog.ShowCustom(i18n.T("core.privileges.title"), func(gtx layout.Context) layout.Dimensions {
		for i := range modes {
...
func (p *CorePage) onPrivilegeModeChange(mode string) {
	...
	p.dialog.ShowCustom(i18n.T("core.btn.apply_setcap"), func(gtx layout.Context) layout.Dimensions {
		if applyBtn.Clicked(gtx) {
			p.dialog.HideCustom()
			go func() {
				p.dialog.ShowLoading(i18n.T("progress.applying_setcap"))
		...
		return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return material.Body2(p.th, i18n.T("core.mode.setcap_prompt")).Layout(gtx)
			}),
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return layout.Inset{Top: unit.Dp(16)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
					return layout.Flex{Axis: layout.Horizontal, Spacing: layout.SpaceBetween}.Layout(gtx,
						layout.Rigid(func(gtx layout.Context) layout.Dimensions {
							return material.Button(p.th, &applyBtn, i18n.T("core.btn.apply")).Layout(gtx)
						}),
						layout.Rigid(func(gtx layout.Context) layout.Dimensions {
							return layout.Inset{Left: unit.Dp(8)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
								return material.Button(p.th, &cancelBtn, i18n.T("dialog.btn.cancel")).Layout(gtx)
							})
						}),
					)
				})
...
func (p *CorePage) onDownloadCore() {
	p.dialog.ShowLoading(i18n.T("progress.checking_version"))
	...
	p.dialog.Show(i18n.T("core.btn.download"), fmt.Sprintf(i18n.T("dialog.download_complete.msg"), ver, path))
	go p.refreshVersions()
}
...
func (p *CorePage) onCheckVersion() {
	...
	p.latestText = i18n.T("core.latest.prefix") + ver
...
func (p *CorePage) showVersionInfoDialog(latest string) {
	...
	if err != nil || currentVer == "" {
		body = i18n.T("dialog.version_check.core_not_installed") + "\n" +
			i18n.T("dialog.version_check.latest") + latest
	} else {
		body = i18n.T("dialog.version_check.current") + currentVer + "\n" +
			i18n.T("dialog.version_check.latest") + latest + "\n"
		if currentVer == latest {
			body += i18n.T("dialog.version_check.latest_installed")
		} else {
			body += i18n.T("dialog.version_check.update_available")
		}
	}
	p.dialog.Show(i18n.T("dialog.version_check.title"), body)
}
